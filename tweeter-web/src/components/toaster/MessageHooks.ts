...
export const useMessageActions = (): MessageActions => {
  ...
  return { 
    ...
    displayErrorMessage: (message: string, bootstrapClasses?: string) =>  displayToast(ToastType.Error, message, 0, bootstrapClasses),
    deleteMessage: (messageId: string) =>  deleteToast(messageId),
